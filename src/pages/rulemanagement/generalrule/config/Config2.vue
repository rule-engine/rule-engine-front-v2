                  ...
                  <a-card :title="cg.name" :bordered="false" v-for="(cg,cgi) in generalRule.conditionGroup"
                    ...
                    <task-group :title="cg.name" group="task" :data-list="generalRule.conditionGroup"
                      ...
                      <a-alert closable
                               ...
                               @close="deleteCondition(cg.conditionGroupCondition,cgc.id)"
...
import {updateCondition, addCondition} from '@/services/condition'
...
export default {
  ...
  methods: {
    ...
    deleteCondition(cgc, conditionId) {
    ...
    addConditionOk(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          // 插入一条记录
          addCondition(this.selectCondition.from).then(res=>{
            if (res.data.data) {
              // 当前条件组内插入一条数据
              this.selectCondition.from.id = res.data.data;
    ...
    addConditionGroup() {
      ...
      let newConditionGroup = {
        id: parseInt(Math.random()*(100000000-90000000+1)+90000000,10),
        name: "条件组",
